...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal):
        ...
        u = self.u.repeat(plane_eq.size(0), plane_eq.size(2) * int(self.upratio), plane_eq.size(3)).to(plane_eq.device)
        u = (u - (self.upratio - 1) * 0.5) / self.upratio
        
        v = self.v.repeat(plane_eq.size(0), plane_eq.size(2), plane_eq.size(3) * int(self.upratio)).to(plane_eq.device)
